...
//   useEffect(() => {
//     // Check if the token is available and fetch user data
//     if (token) {
//       dispatch(getUserData());
//     }
//   }, [dispatch, token]); // Re-run this effect when token changes
...
const Header = ({ onSearch, toggleSidebar, isOpen }) => {
  ...
  const hasChannel = user?.channelId; // Assuming the user data contains a channelId
            ...
            >
              ...
              <ul className="py-2">
                ...
                {hasChannel ? (
                  <li
                    className="px-4 py-2 hover:bg-gray-100 cursor-pointer"
                    onClick={() => navigate(`/channel/${user.channelId}`)} // Navigate to the channel page
                  >
                    Open Channel
                  </li>
                ) : (
